...
######## EC2 Instance ############
##################################
resource "aws_instance" "aha_wordpress" {
  ami               = var.ami
...
######## Key Pair ###########
#############################
resource "tls_private_key" "aha_private_key" {
  algorithm = "RSA"
  rsa_bits  = 4096
}

resource "aws_key_pair" "aha_key_pair" {
  key_name   = var.key_pair
  public_key = tls_private_key.aha_private_key.public_key_openssh
...
############################
output "private_key" {
  value     = tls_private_key.aha_private_key.private_key_pem
  sensitive = true
}

## to see private key pem output, use the command below
